...
echo "regions=/data/database/avera/MedExome_hg19_capture_targets_noann_6col.bed" >> $envlist
echo "cnvregions=/data/database/avera/MedExome_hg19_capture_targets_noann_6col.bed" >> $envlist
echo "bwa_index=/data/database/Homo_sapiens/UCSC/hg19/Sequence/BWAIndex/genome.fa" >> $envlist
echo "anno=/home/anu/repos/annovar" >> $envlist
echo "annodb=/data/database/annovar" >> $envlist
echo "oncodb=/data/database/avera/oncotator_v1_ds_April052016" >> $envlist
...
sed -e 's|'/home/anu/capp-seq-docker/env.list'|'$envlist'|g' $repo/variant.sh > $out/$sample.variant.sh
